/**
 * Local asset storage helper used in single-user development.
 */
...
export async function readStoredAsset(key: string): Promise<{ body: ReadableStream<Uint8Array> } | null> {
  const fileName = key.replace(/\//g, "_");
  const filePath = join(LOCAL_UPLOAD_DIR, fileName);
